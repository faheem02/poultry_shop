...
foreach ($types as $t) {
    $stmt = $pdo->prepare("
        SELECT
            COALESCE(SUM(CASE WHEN transaction_type IN ('opening','purchase') THEN birds_count ELSE 0 END), 0) AS in_birds,
            COALESCE(SUM(CASE WHEN transaction_type IN ('opening','purchase') THEN weight_kg ELSE 0 END), 0) AS in_weight,
            ...
            COALESCE(SUM(CASE WHEN transaction_type IN ('opening','purchase') THEN amount ELSE 0 END), 0) AS in_amount,
...
foreach ($ledger_asc as $l) {
    if (in_array($l['transaction_type'], ['opening','purchase'])) {
